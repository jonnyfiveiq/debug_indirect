...
No `event_query.yml` file or database access needed. The script analyses the actual event stream data from any job, regardless of which collection ran it.
...
python3.11 ./diagnose_indirect_nodes.py --discover \
  ...
  --token <oauth_token> --no-verify-ssl
```
...
2. **Discovers jobs** that ran modules from collections with `event_query.yml` files
